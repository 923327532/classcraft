...
<head>
    ...
    <title>ClassCraft - Sanador</title>
...
<body>
        ...
        <main class="flex-grow flex items-center justify-center p-6 sm:p-10 lg:p-16">
            <div class="flex flex-col md:flex-row items-center md:items-center justify-center gap-6 w-full max-w-6xl mx-auto">
                ...
                <div id="powerSelectionCard" class="bg-gray-50 rounded-2xl shadow-2xl p-10 sm:p-12 md:p-16 text-center w-full md:w-3/5 lg:w-3/5 transform transition-all duration-500 hover:scale-[1.01] hover:shadow-3xl border border-gray-100 flex-shrink-0">
                    ...
                    <div class="w-24 h-1 bg-emerald-500 rounded-full mx-auto mt-8 opacity-75"></div>
                ...
                <div id="powerDetailsCard" class="bg-emerald-50 rounded-2xl shadow-2xl p-6 sm:p-8 md:p-10 text-center w-full md:w-2/5 lg:w-2/5 border border-emerald-200 transition-opacity duration-500 flex flex-col justify-between items-center flex-grow h-auto md:min-h-[400px] opacity-0 pointer-events-none">
    ...
    <script>
        ...
        const powerImagesBaseUrl = "{{ asset('images/powers/sanadores') }}/"; 
        ...
        function displayPowerDetails(power) {
            ...
            powerDetailsCard.innerHTML = `
                <img src="${powerImagesBaseUrl}${power.imagePath.split('/').pop()}" alt="${power.name}" class="w-24 h-24 mb-4 rounded-full object-cover mx-auto border-4 border-emerald-400 power-icon" onerror="this.onerror=null;this.src='https://placehold.co/100x100/CCCCCC/666666?text=Error';">
                <h4 class="text-3xl font-bold text-emerald-900 mb-2">${power.name}</h4>
                <p class="text-lg text-gray-700 mb-4">${power.description}</p>
                <p class="text-md text-emerald-600">- ${power.ppCost} AP</p>
                <button id="confirmSelectionButton" class="mt-8 px-8 py-3 bg-emerald-600 text-white font-bold rounded-full shadow-lg hover:bg-emerald-700 transition-all duration-300 transform hover:-translate-y-1 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-emerald-300">Confirmar Elección</button>
            `;
                    ...
                    groupedPowers[type].forEach(power => {
                        ...
                        powerCard.className = `
                            ...
                            ${isLocked 
                                ...
                                : 'border-emerald-300 bg-emerald-50 text-emerald-800 cursor-pointer hover:bg-emerald-100 hover:shadow-md'
                            }
                        `;
