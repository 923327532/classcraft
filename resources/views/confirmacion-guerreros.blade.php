<x-app-layout>
    ...
    <body>
            ...
            <main class="flex-grow flex items-center justify-center p-6 sm:p-10 lg:p-16">
                <div class="flex flex-col md:flex-row items-center md:items-center justify-center gap-6 w-full max-w-6xl mx-auto">
                    ...
                    <div id="powerSelectionCard" class="bg-gray-50 rounded-2xl shadow-2xl p-10 sm:p-12 md:p-16 text-center w-full md:w-3/5 lg:w-3/5 transform transition-all duration-500 hover:scale-[1.01] hover:shadow-3xl border border-gray-100 flex-shrink-0">
                        ...
                        <h3 class="text-5xl sm:text-6xl font-extrabold text-gray-800 leading-tight mb-6">
                            Has elegido a: <span class="text-black">⚔️{{ ucfirst($personaje) }}</span>
                        </h3>
                    ...
                    <div id="powerDetailsCard" class="bg-blue-50 rounded-2xl shadow-2xl p-6 sm:p-8 md:p-10 text-center w-full md:w-2/5 lg:w-2/5 border border-blue-200 transition-opacity duration-500 flex flex-col justify-between items-center flex-grow h-auto md:min-h-[400px] opacity-0 pointer-events-none">
        ...
        <script>
            ...
            const powerImagesBaseUrl = "{{ asset('images/powers') }}/"; 
            ...
            function displayPowerDetails(power) {
                ...
                console.log('power.imagePath para detalles:', power.imagePath);
                console.log('URL de imagen final intentando cargar para detalles:', powerImagesBaseUrl + power.imagePath.split('/').pop());

                powerDetailsCard.innerHTML = `
                    <img src="${powerImagesBaseUrl}${power.imagePath.split('/').pop()}" alt="${power.name}" class="w-24 h-24 mb-4 rounded-full object-cover mx-auto border-4 border-blue-400 power-icon" onerror="this.src='https://placehold.co/100x100/CCCCCC/666666?text=Error';">
                    <h4 class="text-3xl font-bold text-blue-900 mb-2">${power.name}</h4>
                    <p class="text-lg text-gray-700 mb-4">${power.description}</p>
                    <p class="text-md text-blue-600">- ${power.ppCost} AP</p>
                    <button id="confirmSelectionButton" class="mt-8 px-8 py-3 bg-indigo-600 text-white font-bold rounded-full shadow-lg hover:bg-indigo-700 transition-all duration-300 transform hover:-translate-y-1 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-indigo-300">Confirmar Elección</button>
                `;
            ...
            function renderPowers() {
                ...
                if (powers.length > 0) {
                    ...
                    sectionTitle.textContent = 'Poderes Básicos (Nivel 1)'; 
                    ...
                    powers.forEach(power => {
                        ...
                        powerCard.className = `
                            ...
                            ${isLocked 
                                ...
                                : 'border-blue-300 bg-blue-50 text-blue-800 cursor-pointer hover:bg-blue-100 hover:shadow-md'
                            }
                        `;
                        powerCard.dataset.locked = isLocked; 
                        
                        console.log('power.imagePath para tarjeta:', power.imagePath);
                        console.log('URL de imagen final intentando cargar para tarjeta:', powerImagesBaseUrl + power.imagePath.split('/').pop());

                        powerCard.innerHTML = `
                            <img src="${powerImagesBaseUrl}${power.imagePath.split('/').pop()}" alt="${power.name}" class="w-16 h-16 mb-2 rounded-full object-cover mx-auto power-icon" onerror="this.src='https://placehold.co/60x60/CCCCCC/666666?text=Error';">
                    ...
                    powersContainer.innerHTML = '<p class="text-xl text-gray-600 text-center">No hay poderes disponibles para tu nivel.</p>';
                }
            }

            document.addEventListener('DOMContentLoaded', () => {
                console.log("Poderes recibidos en JS (para depuración):", powers); 
                renderPowers();
            });
        </script>
    </body>
